...
<title>vineyard for modify</title>
</head>

<body>
	<%= request.getRequestURI() %> <br>
	vineyard for modify<br>
	...
	<%
		...
		try {
			...
			stmt = conn.prepareStatement(
					"SELECT * FROM vineyard WHERE vineyard_ID=\""+vineyard_ID+"\"");
			
			rs = stmt.executeQuery();
			
			System.out.println(rs + "<-- rs vineyard_modify_form.jsp");
		rs.next();
			
%>
	<form action="<%=request.getContextPath() %>/vineyard/vineyard_modify.jsp">
		<table border = "1">
			<tr>
				<td>vineyard_ID</td>
				<td><input type="text" name="vineyard_ID" value=<%=rs.getString("vineyard_ID") %> required size="10"></td>
			<tr>
			<tr>
				<td>owner</td>
